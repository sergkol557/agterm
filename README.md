...
Sidebar session rows support Shift-click range selection and Cmd-click toggling for batch work. Right-clicking inside a multi-selection keeps the batch for Flag/Unflag, Close, and Move to; right-clicking outside narrows to the clicked row. Dragging from a selected row moves the selected sessions as one ordered block. **Copy Name** — under Rename on a session or workspace row, single-target — puts the row's name on the clipboard. **Duplicate Session** — in a single session's context menu, under Copy Name — opens a fresh session in the same workspace, right after that one, in its current directory (a plain new shell: only the directory carries over, nothing else about the session does).
...
- The action palette shows built-in shortcuts as macOS glyphs (⌘⇧E) and custom commands as raw kitty syntax (`cmd+shift+e`). A built-in with alternatives shows them all, space-separated, with a sequence's chords joined by `>` as in the config (⌘T ⌃A>T), in the palette and in the toolbar and sidebar tooltips alike.
