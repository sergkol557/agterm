...
- Commands:
  - `scripts/sync.sh`: fetch/merge upstream, release build, deploy, install-ctl, clean, push.
  ...
  - `make prep|build|run|release|deploy|build-ctl|install-ctl|sync|test|test-app|lint|clean`; `make dist VERSION=x.y.z [PUBLISH=1]`.
